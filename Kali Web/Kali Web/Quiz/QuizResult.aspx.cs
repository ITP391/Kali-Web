    ...
    public partial class QuizResult : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
                ...
                .ConnectionStrings["localdbConnectionString1"].ConnectionString))
            {
                ...
                command.Parameters.AddWithValue("CorrectAnswers", answersList.Count(x => x.ChosenAnswer == x.CorrectAnswer).ToString());
                command.Parameters.AddWithValue("TotalAnswers", answersList.Count.ToString()); 
                command.Parameters.AddWithValue("Score",  answersList.Count(x => x.ChosenAnswer == x.CorrectAnswer) / answersList.Count * 10);
            ...
            correctAnswersLabel.Text = answersList.Count(x => x.ChosenAnswer == x.CorrectAnswer).ToString();
        }
    }
}